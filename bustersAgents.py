...
from game import Actions, Agent, Directions, Directions
import game
import inference
...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghostDist in livingGhostPositionDistributions:
            bestProb = 0
            bestPos = None
            for ghostPos, prob in ghostDist.items():
                if prob >= bestProb:
                    bestPos = ghostPos
                    bestProb = prob
            ghostPositions[bestPos] = bestProb
        
        minDist = 9999999
        bestGhost = None
        for ghostPos, prob in ghostPositions.items():          
        ...
        bestMove = None
        for action in legal:
            newPos = game.Actions.getSuccessor( pacmanPosition, action )
            dist = self.distancer.getDistance( newPos, bestGhost )
            if ( dist < minDist ):
                minDist = dist
                bestMove = action
